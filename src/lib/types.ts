...
export const experienceSchema = z.object({
  ...
  canWorkWithBedBound: z.boolean().optional().default(false),
...
export const newHireChecklistSchema = z.object({
    driversLicenseReceived: z.boolean().default(false),
    driversLicenseComment: z.string().trim().optional(),
    dmvRecordReceived: z.boolean().default(false),
    dmvRecordComment: z.string().trim().optional(),
    carInsuranceReceived: z.boolean().default(false),
    carInsuranceComment: z.string().trim().optional(),
    carRegistrationReceived: z.boolean().default(false),
    carRegistrationComment: z.string().trim().optional(),
    ssnCardReceived: z.boolean().default(false),
    ssnCardComment: z.string().trim().optional(),
    hcaClearanceReceived: z.boolean().default(false),
    hcaClearanceComment: z.string().trim().optional(),
    liveScanLetterReceived: z.boolean().default(false),
    liveScanLetterComment: z.string().trim().optional(),
    tbTestResultsReceived: z.boolean().default(false),
    tbTestResultsComment: z.string().trim().optional(),
    vaccineRecordReceived: z.boolean().default(false),
    vaccineRecordComment: z.string().trim().optional(),
    physicalFormReceived: z.boolean().default(false),
    physicalFormComment: z.string().trim().optional(),
    cprFirstAidReceived: z.boolean().default(false),
    cprFirstAidComment: z.string().trim().optional(),
    otherReceived: z.boolean().default(false),
    otherComment: z.string().trim().optional(),
});
export type NewHireChecklistFormData = z.infer<typeof newHireChecklistSchema>;
...
export const caregiverFormSchema = generalInfoSchema
  ...
  .merge(newHireChecklistSchema.partial())
  .extend({ 
...
export const clientSignupFormSchema = z.object({
  ...
  clientAddress: z.string().trim().min(1, "Address is required."),
  clientCity: z.string().trim().min(1, "City is required."),
  clientState: z.string().trim().min(1, "State is required."),
  clientPostalCode: z.string().trim().min(1, "Postal Code is required."),
  ...
  emergencyContactHomePhone: z.string().trim().min(1, "Home Phone is required."),
  ...
  homemakerCompanion: z.boolean().default(false),
  ...
  rateCardDate: z.string().trim().optional(),
  policyNumber: z.string().trim().optional(),
  policyPeriod: z.string().trim().optional(),
  clientInitials: z.string().trim().optional(),
  receivedPrivacyPractices: z.boolean().default(false),
  receivedClientRights: z.boolean().default(false),
  receivedTransportationWaiver: z.boolean().default(false),
  receivedPaymentAgreement: z.boolean().default(false),
  receivedAdditionalDisclosures: z.boolean().default(false),
  clientSignature: z.string().trim().optional(),
  clientPrintedName: z.string().trim().optional(),
  clientSignatureDate: z.string().trim().optional(),
  clientRepresentativeSignature: z.string().trim().optional(),
  clientRepresentativePrintedName: z.string().trim().optional(),
  clientRepresentativeSignatureDate: z.string().trim().optional(),
  firstLightRepresentativeSignature: z.string().trim().optional(),
  firstLightRepresentativeTitle: z.string().trim().optional(),
  firstLightRepresentativeSignatureDate: z.string().trim().optional(),
  companionCare_mealPreparation: z.boolean().default(false),
  companionCare_cleanKitchen: z.boolean().default(false),
  companionCare_assistWithLaundry: z.boolean().default(false),
  companionCare_dustFurniture: z.boolean().default(false),
  companionCare_assistWithEating: z.boolean().default(false),
  companionCare_provideAlzheimersRedirection: z.boolean().default(false),
  companionCare_assistWithHomeManagement: z.boolean().default(false),
  companionCare_preparationForBathing: z.boolean().default(false),
  companionCare_groceryShopping: z.boolean().default(false),
  companionCare_cleanBathrooms: z.boolean().default(false),
  companionCare_changeBedLinens: z.boolean().default(false),
  companionCare_runErrands: z.boolean().default(false),
  companionCare_escortAndTransportation: z.boolean().default(false),
  companionCare_provideRemindersAndAssistWithToileting: z.boolean().default(false),
  companionCare_provideRespiteCare: z.boolean().default(false),
  companionCare_stimulateMentalAwareness: z.boolean().default(false),
  companionCare_assistWithDressingAndGrooming: z.boolean().default(false),
  companionCare_assistWithShavingAndOralCare: z.boolean().default(false),
  companionCare_other: z.string().trim().optional(),
  personalCare_provideAlzheimersCare: z.boolean().default(false),
  personalCare_provideMedicationReminders: z.boolean().default(false),
  personalCare_assistWithDressingGrooming: z.boolean().default(false),
  personalCare_assistWithBathingHairCare: z.boolean().default(false),
  personalCare_assistWithFeedingSpecialDiets: z.boolean().default(false),
  personalCare_assistWithMobilityAmbulationTransfer: z.boolean().default(false),
  personalCare_assistWithIncontinenceCare: z.boolean().default(false),
  personalCare_assistWithOther: z.string().trim().optional(),
  servicePlanClientInitials: z.string().trim().optional(),
  officeTodaysDate: z.string().trim().optional(),
  officeReferralDate: z.string().trim().optional(),
  officeInitialContactDate: z.string().trim().optional(),
  ...
  transportationWaiverWitnessSignature: z.string().trim().optional(),
  transportationWaiverDate: z.string().trim().optional(),
...
export const finalizationSchema = clientSignupFormSchema.extend({
  hourlyRate: z.coerce.number().min(1, "Hourly rate is required for finalization."),
  minimumHoursPerShift: z.coerce.number().min(1, "Minimum hours per shift is required."),
  rateCardDate: z.string().trim().min(1, "Rate card date is required."),
  contractStartDate: z.string().trim().min(1, "Contract start date is required."),
  clientSignature: z.string().trim().min(1, "Client signature is required."),
  clientSignatureDate: z.string().trim().min(1, "Client signature date is required."),
  clientPrintedName: z.string().trim().min(1, "Client printed name is required."),
  clientInitials: z.string().trim().min(1, "Client initials are required."),
  servicePlanClientInitials: z.string().trim().min(1, "Service Plan initials are required."),
  agreementClientSignature: z.string().trim().min(1, "Agreement client signature is required."),
  agreementSignatureDate: z.string().trim().min(1, "Agreement signature date is required."),
  agreementRepSignature: z.string().trim().min(1, "Agreement representative signature is required."),
  agreementRepDate: z.string().trim().min(1, "Agreement representative date is required."),
});

export const tppFinalizationSchema = clientSignupFormSchema.extend({
  payor: z.string().trim().min(1, "Payor name is required for finalization."),
  contractStartDate: z.string().trim().min(1, "Contract start date is required."),
  clientSignature: z.string().trim().min(1, "Client signature is required."),
  clientSignatureDate: z.string().trim().min(1, "Client signature date is required."),
  clientPrintedName: z.string().trim().min(1, "Client printed name is required."),
...
export const clientSignaturePayloadSchema = z.object({
  ...
  clientSignatureDate: z.string().min(1, "Client signature date is required."),
  clientPrintedName: z.string().min(1, "Client printed name is required."),
  clientInitials: z.string().min(1, "Client initials are required."),
  servicePlanClientInitials: z.string().min(1, "Service Plan initials are required."),
  agreementClientSignature: z.string().min(1, "Agreement client signature is required."),
  agreementSignatureDate: z.string().min(1, "Agreement signature date is required."),
  ...
  clientRepresentativeSignatureDate: z.string().optional(),
  ...
  transportationWaiverDate: z.string().optional(),
});
