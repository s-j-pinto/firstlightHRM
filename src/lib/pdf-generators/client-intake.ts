...
export async function generateClientIntakePdf(formData: ClientSignupFormData, formType: 'private' | 'tpp' = 'private'): Promise<Buffer> {
    try {
        ...
        const startDate = formData.contractStartDate ? (typeof formData.contractStartDate === 'string' ? formData.contractStartDate : format(formData.contractStartDate, 'MM/dd/yyyy')) : '';
