...
function drawWrappedText(page: any, text: string | undefined, font: PDFFont, fontSize: number, x: number, y: number, maxWidth: number, lineHeight: number): number {
    ...
    for (const word of words) {
        ...
        if (testWidth > maxWidth && line.length > 0) {
            page.drawText(line, { x: x, y: currentY, font, size: fontSize, color: rgb(0,0,0) });
            line = word + ' ';
            currentY -= lineHeight;
        } else {
            line = testLine;
    ...
    if (line.trim().length > 0) {
        page.drawText(line.trim(), { x: x, y: currentY, font, size: fontSize, color: rgb(0,0,0) });
    }
...
export async function generateReferenceVerificationPdf(formData: any): Promise<{ pdfData?: string; error?: string }> {
    try {
        ...
        y -= (logoDims.height) - 30; // REDUCED SPACE
        ...
        drawText(page, pleasePrint, {x: leftMargin, y: y, font, size: 7}); // Font size reduced
        y -= 5;
        page.drawLine({ start: { x: leftMargin, y: y }, end: { x: rightMargin, y: y }, thickness: 1 });
        y -= 15;

        drawText(page, `Applicant’s First Name Middle Last: ${formData.fullName || ''}`, {x: leftMargin, y, font, size: 7}); // Font size reduced
        y -= 15;
        ...
        y = drawWrappedText(page, permissionText, font, 7, leftMargin, y, contentWidth, 9); // Font size reduced
        y -= 20;
        
        if (formData.applicantSignature) {
            drawSignature(page, formData.applicantSignature, leftMargin + 80, y - 5, 150, 30, pdfDoc);
        }
        page.drawLine({ start: { x: leftMargin, y: y - 10 }, end: { x: leftMargin + 300, y: y - 10 }, thickness: 0.5 });
        drawText(page, "Signature", {x: leftMargin, y: y-20, font, size: 7}); // Font size reduced
        ...
        if (sigDate) {
             drawText(page, `Date: ${sigDate}`, {x: leftMargin + 350, y, font, size: 7}); // Font size reduced
        }
        page.drawLine({ start: { x: leftMargin + 340, y: y-10 }, end: { x: leftMargin + 500, y: y - 10 }, thickness: 0.5 });
        drawText(page, "Date", {x: leftMargin + 340, y: y-20, font, size: 7}); // Font size reduced
        y -= 30;

        const employerBoxStartY = y;
        drawText(page, "FORMER EMPLOYER CONTACT INFORMATION", {x: leftMargin, y, font: boldFont, size: 7}); // Font size reduced
        y -= 15;

        const drawTwoColumnField = (label1: string, value1: string | undefined, label2: string, value2: string | undefined) => {
            const smallFontSize = 7; // Font size reduced
        ...
        drawText(page, "REFERENCE INFORMATION", {x: leftMargin, y, font: boldFont, size: 7}); // Font size reduced
        y -= 10;
        drawText(page, "Please rate yourself in the following categories as you feel your former supervisor will rate you:", {x: leftMargin, y, font, size: 7}); // Font size reduced
        y -= 10;

        const drawRating = (label: string, value: string | undefined) => {
            const smallFontSize = 7; // Font size reduced
            y = drawWrappedText(page, label, boldFont, smallFontSize, leftMargin + 5, y, contentWidth - 10, 8); // line height adjusted
            y -= 3; // REDUCED SPACE
        ...
        const drawYesNo = (label: string, value: string | undefined, yPos: number, xPos: number) => {
            drawText(page, `${label}: ${value || ''}`, {x: xPos, y: yPos, font, size: 7}); // Font size reduced
        };
        ...
        if (formData.wasDisciplined === 'Yes' && formData.disciplineExplanation) {
            y = drawWrappedText(page, `Explain: ${formData.disciplineExplanation}`, font, 7, leftMargin, y, contentWidth, 9); // Font size reduced
        }
        y-=10;
        drawWrappedText(page, "Someone from FirstLight HomeCare will be following up with your shortly regarding the employment reference verification check. If you have any questions, please call: 909-321-4466", font, 7, leftMargin, y, contentWidth, 9); // Font size reduced
...
export async function generateLic508Pdf(formData: any): Promise<{ pdfData?: string; error?: string }> {
    try {
        ...
        const drawHeaderAndFooter = (page: any, pageNum: number) => {
            ...
            const footerY = 30;
            const fontSize = 10; // increased from 8
        ...
        const contentWidth = rightMargin - leftMargin;
        const mainFontSize = 10.5; // increased from 9.5
        const titleFontSize = 15; // increased from 14
        const smallFontSize = 10; // increased from 8
        ...
        page = pages[1];
        y = height - 70;
        const labelFontSize = 10; // increased from 8
        ...
        const instructionsLicensees = "Instructions to Licensees:";
        drawText(page, instructionsLicensees, { x: leftMargin, y, font: boldFont, size: mainFontSize });
        y -= lineHeight;
        ...
        const instructionsRegional = "Instructions to Regional Offices and Foster Family Agencies:";
        drawText(page, instructionsRegional, { x: leftMargin, y, font: boldFont, size: mainFontSize });
        y -= lineHeight;
        ...
        drawText(page, "Privacy Notice", { x: (width / 2) - (boldFont.widthOfTextAtSize("Privacy Notice", titleFontSize) / 2), y, font: boldFont, size: titleFontSize });
        y -= 15;
        drawText(page, "As Required by Civil Code § 1798.17", { x: (width / 2) - (font.widthOfTextAtSize("As Required by Civil Code § 1798.17", smallFontSize) / 2), y, font, size: smallFontSize });
        y -= 25;

        drawText(page, "Collection and Use of Personal Information.", { x: leftMargin, y, font: boldFont, size: mainFontSize }); y -= lineHeight;
        y = drawWrappedText(page, "The California Justice Information Services (CJIS) Division in the Department of Justice (DOJ) collects the information requested on this form as authorized by Penal Code sections 11100-11112; Health and Safety Code sections 1522, 1569.10-1569.24, 1596.80-1596.879; Family Code sections 8700-87200; Welfare and Institutions Code sections 16500-16523.1; and other various state statutes and regulations. The CJIS Division uses this information to process requests of authorized entities that want to obtain information as to the existence and content of a record of state or federal convictions to help determine suitability for employment, or volunteer work with children, elderly, or disabled; or for adoption or purposes of a license, certification, or permit. In addition, any personal information collected by state agencies is subject to the limitations in the Information Practices Act and state policy. The DOJ’s general privacy policy is available at http://oag.ca.gov/privacy-policy.", font, mainFontSize, leftMargin, y, contentWidth, lineHeight); y -= 20;

        drawText(page, "Providing Personal Information.", { x: leftMargin, y, font: boldFont, size: mainFontSize }); y -= lineHeight;
        y = drawWrappedText(page, "All the personal information requested in the form must be provided. Failure to provide all the necessary information will result in delays and/or the rejection of your request. Notice is given for the request of the Social Security Number (SSN) on this form. The California Department of Justice uses a person’s SSN as an identifying number. The requested SSN is voluntary. Failure to provide the SSN may delay the processing of this form and the criminal record check.", font, mainFontSize, leftMargin, y, contentWidth, lineHeight); y -= 20;

        drawText(page, "Access to Your Information.", { x: leftMargin, y, font: boldFont, size: mainFontSize }); y -= lineHeight;
        y = drawWrappedText(page, "You may review the records maintained by the CJIS Division in the DOJ that contain your personal information, as permitted by the Information Practices Act. See below for contact information.", font, mainFontSize, leftMargin, y, contentWidth, lineHeight); y -= 20;

        drawText(page, "Possible Disclosure of Personal Information.", { x: leftMargin, y, font: boldFont, size: mainFontSize }); y -= lineHeight;
        y = drawWrappedText(page, "In order to be licensed, work at, or be present at, a licensed facility/organization, or be placed on a registry administered by the Department, the law requires that you complete a criminal background check. (Health and Safety Code sections 1522, 1568.09, 1569.17 and 1596.871). The Department will create a file concerning your criminal background check that will contain certain documents, including personal information that you provide. You have the right to access certain records containing your personal information maintained by the Department (Civil Code section 1798 et seq.). Under the California Public Records Act (Government Code section 6250 et seq.), the Department may have to provide copies of some of the records in the file to members of the public who ask for them, including newspaper and television reporters.", font, mainFontSize, leftMargin, y, contentWidth, lineHeight); y -= 20;
        
        y = drawWrappedText(page, "NOTE: IMPORTANT INFORMATION", boldFont, mainFontSize, leftMargin, y, contentWidth, lineHeight); y-= lineHeight;
        ...
        drawText(page, "Contact Information", { x: (width / 2) - (boldFont.widthOfTextAtSize("Contact Information", titleFontSize) / 2), y, font: boldFont, size: titleFontSize }); y -= 25;
        ...
        drawText(page, "Applicant Notification and Record Challenge", { x: leftMargin, y, font: boldFont, size: mainFontSize }); y -= lineHeight;
