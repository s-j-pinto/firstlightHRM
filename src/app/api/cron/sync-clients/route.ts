...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (parseResult.errors.length > 0) {
        ...
        throw new Error('Failed to parse active clients CSV.');
    }

    const result = await processClientUpload(parseResult.data as any[]);
    ...
    console.error('[CRON-ERROR] /api/cron/sync-clients:', error);
